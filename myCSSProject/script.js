...
myApp.controller("MainController", [
  ...
  function ($scope) {
    ...
    $scope.defaultDisplay = function (length) {
      ...
      focusCube["background-color"] = "#198754";
    };
    ...
    $scope.detectItem = function (target) {
      ...
      for (let i = 0; i < $scope.itemArray.length; i++) {
        i == target
          ? ($scope.itemArray[i].visible = true)
          : ($scope.itemArray[i].visible = false);
        if (i !== target) {
          $scope.itemArray[i]["background-color"] = "rgb(41, 124, 192)";
        }
      }
    };

    $scope.setDirection = function (direction) {
      if (direction !== null && display !== undefined)
        $scope.containerStyle["flex-direction"] = direction;
    };

    $scope.setDisplay = function (display) {
      if (display !== null && display !== undefined)
        $scope.containerStyle.display = display;
    };

    $scope.setJustifyContent = function (position) {
      if (position !== null && display !== undefined)
        $scope.containerStyle["justify-content"] = position;
    };

    $scope.setWrap = function (option) {
      if (option !== null && display !== undefined)
        $scope.containerStyle["flex-wrap"] = option;
    };

    $scope.setAlignContent = function (position) {
      if (position !== null && display !== undefined)
        $scope.containerStyle["align-content"] = position;
    };

    $scope.setAlignItems = function (position) {
      if (position !== null && display !== undefined)
        $scope.containerStyle["align-items"] = position;
    };
